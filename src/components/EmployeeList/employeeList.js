  ...
  return (
    <div className="employee-list">
      ...
      <div className="employee-list__container">
        {employee?.length > 0 &&
          employee.map((item, id) => <div key={id} style={{fontSize : `${employee[id].popularity * 0.8}rem`}} className={`employee-list__container__item ${currentId === id && 'active'}` } onClick={onChange(id)}>{item?.name}</div>)}
      </div>
    </div>
  )
}
